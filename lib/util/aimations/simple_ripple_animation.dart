...
class RippleAnimationState extends State<RippleAnimation> with TickerProviderStateMixin {
  ...
  Widget build(final BuildContext context) => CustomPaint(
    painter: CirclePainter(_controller, color: widget.color, minRadius: widget.minRadius, wavesCount: widget.ripplesCount + 2),
    child: widget.child,
  );
}
...
class CirclePainter extends CustomPainter {
  ...
  CirclePainter(this.animation, {required this.wavesCount, required this.color, this.minRadius}) : super(repaint: animation);
  ...
  void paint(final Canvas canvas, final Size size) {
    ...
    for (int wave = 0; wave <= wavesCount; wave++) {
      circle(canvas, rect, minRadius, wave, animation!.value, wavesCount, color);
    }
  }

  /// animating the opacity according to min radius and waves count.
  void circle(final Canvas canvas, final Rect rect, final double? minRadius, final int wave, final double value, final int? length, final Color circleColor) {
  ...
  @override
  bool shouldRepaint(final CirclePainter oldDelegate) => true;
}
